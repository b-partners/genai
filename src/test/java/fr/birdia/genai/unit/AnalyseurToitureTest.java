...
class AnalyseurToitureTest {
  ...
  @Test
  void defaults_revetement_when_missing_from_the_request() {
    // Unlike the other optional fields, the prompt template has no th:if guard around
    // ${revetement}, so a null value must be defaulted before it reaches the template.
    when(chat.apply(contains("Toiture en bon état"))).thenReturn("<section>ok</section>");
    ...
    var toit =
        new Toit(
            ...
            true,
            "RAS",
            EtatApparent.fromLibelle("Toiture en bon état"),
            3.0,
            "proximité littorale");

    subject.apply(toit);

    verify(chat).apply(contains("Revêtement principal : Non renseigné"));
  }
}
